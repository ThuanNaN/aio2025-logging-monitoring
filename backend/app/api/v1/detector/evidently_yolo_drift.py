...
class EvidentlyYOLODriftDetector:
    ...
    def __init__(
        ...
        Args:
            reference_window_size: Number of samples to keep as reference data
        ...
        self.numeric_features = [
            'brightness', 'num_detections', 'avg_confidence'
        ]
    ...
    def add_sample(self, 
        ...
        Args:
            ...
            embedding_features: Optional embedding features (flattened)
        """
        sample = {
        ...
        if embedding_features is not None:
            for i, val in enumerate(embedding_features.flatten()[:10]):  # Limit to 10 features
                sample[f'embedding_{i}'] = val
    ...
    def detect_drift(self, window_size: int = 50) -> Dict[str, Any]:
        """
        Detect drift between reference and current data windows
        
        Args:
            window_size: Size of the current window to compare against reference (deprecated, uses detection_window_size)
        ...
        try:
            ...
            for col_name, col_data in drift_by_columns.items():
                if col_name in self.numeric_features:
